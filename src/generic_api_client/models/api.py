...
class RetryConfig(BaseModel):
    ...
    max_retries: int = Field(
        0, description="The number of retries allowed for a request"
    )
...
class APICommonRequestFields(BaseModel):
    ...
    root_url: str = Field(
        "", description="The api root url", examples=["ocs/v2.php/cloud"]
    )
    method: HTTPMethod | None = Field(
        None, description="Default method for API requests"
    )
    headers: dict[str, Any] | None = Field(
        None, description="Default headers for API requests"
    )
    cookies: dict[str, Any] | None = Field(
        None, description="Default cookies for API requests"
    )
    ...
    requires_version: bool = Field(
        False, description="Either the API requires to know the target version"
    )
    ...
    retries: RetryConfig = Field(
        description="The configuration to retry a request", default_factory=RetryConfig
    )
